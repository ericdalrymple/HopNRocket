...
: MonoBehaviour
{
	...
	void Start()
	{
		...
		objectBuffer = GameObject.Find( "lblBestScoreValue" );
		if( null != objectBuffer )
		{
			m_BestScoreText = objectBuffer.GetComponent<Text>();
			if( null != m_BestScoreText )
			{
				//-- Immediately set the value to the best score
				m_ConcatBuffer.Length = 0;
				m_ConcatBuffer.Append( ScoreManager.instance.GetBestScore() );
				m_BestScoreText.text = m_ConcatBuffer.ToString();
			}
		}

		objectBuffer = GameObject.Find( "lblScoreValue" );
		if( null != objectBuffer )
		{
			m_ScoreText = objectBuffer.GetComponent<Text>();
		}

		objectBuffer = GameObject.Find( "lblToken1Quantity" );
		if( null != objectBuffer )
		{
			m_TokenText = objectBuffer.GetComponent<Text>();
		}
	}
	...
	void UpdateScoreLabel()
	{
		...
		m_NewScore = ScoreManager.instance.GetTotalScore();
